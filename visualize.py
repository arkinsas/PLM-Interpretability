...
def plot_head_attention(model, alphabet, sequence, layer, head, contact_pair=None, device="cpu", save_path=None):
    """
    ...
    with torch.no_grad():
        ...
        if attn.shape[0] == num_layers:
            ...
            print(f"Debug Info: Tensor shape is {attn.shape}, Num Layers is {num_layers}")
            raise ValueError("Could not determine layer dimension in attention tensor.")

    # --- PLOTTING ---
    plt.figure(figsize=(10, 8))

    # Use viridis for contrast, square=True to keep aspect ratio
...
def main():
    ...
    parser.add_argument("--protein", type=str, choices=[p.name for p in Protein],
    ...
    for protein in proteins_to_process:
        ...
        save_path = os.path.join(args.output_dir, f"{safe_name}_L{args.layer}H{args.head}.png")
