...
def run_contact_sweep(
    ...
    pair_results = [
        {"index": idx, "clean": c, "corrupted": d, "contact_idx": c_idx, "head_results": []}
    ...
    for start in range(0, len(pairs), batch_size):
        ...
        _, _, clean_tokens = batch_converter([(f"c{i}", s) for i, s in enumerate(clean_seqs)])
        
        _, _, corrupted_tokens = batch_converter([(f"d{i}", s) for i, s in enumerate(corrupted_seqs)])
